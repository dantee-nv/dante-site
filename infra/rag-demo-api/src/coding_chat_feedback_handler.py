...
MAX_QUESTION_LENGTH = max(20, int(os.getenv("CODING_CHAT_MAX_MESSAGE_LENGTH", "1200")))
MAX_ANSWER_LENGTH = max(500, int(os.getenv("CODING_CHAT_MAX_REPLY_LENGTH", "12000")))
MAX_NOTE_LENGTH = 1000
MAX_USER_AGENT_LENGTH = 500
FEEDBACK_TABLE_NAME = os.getenv("FEEDBACK_TABLE_NAME", "").strip()
VALID_MODES = {"hints", "full_solution"}
DEMO_TYPE = "coding-challenge-chatbot"
...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.exception(
            "coding_chat_feedback_failed error_type=%s error_message=%s",
            type(error).__name__,
            str(error),
        )
        return _json_response(
            500,
            {"message": "Failed to record coding challenge feedback. Please try again."},
        )
